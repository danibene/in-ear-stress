...
def specificity_score(y_true, y_pred, pos_label=None):
    """Specificity = TN / (TN + FP)"""
    if pos_label is None:
        labels = None
    else:
        labels = [l for l in np.unique(y_true) if l != pos_label] + [pos_label]
...
if __name__ == "__main__":
    ...
    results_df_name = "results_comptasks_" + time.strftime("%Y%m%d-%H%M%S") + ".csv"
    ...
    if not pathlib.Path(results_df_path).is_file():
        ...
        task_labels = ["CPT", "MENTAL", "CPT + MENTAL"]
        ...
        header = True
        eval_ind = 0
        for train_task_label in task_labels:
            train_task_names = train_task_label.split(" + ")
            for val_task_label in task_labels:
                val_task_names = val_task_label.split(" + ")
