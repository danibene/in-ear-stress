...
parser.add_argument(
    "--use_matlab", default=False, action=argparse.BooleanOptionalAction
)
...
if __name__ == "__main__":
    ...
    if root_in_path is None:
        root_in_path = pathlib.Path(
            code_paths["repo_path"], "local_data", "test_model_hum_example"
        )
    file_path = pathlib.Path(root_in_path, "sub-202102239_task-rest_IEML.wav")

    #### STEP 1: LOAD SIGNAL
    # load raw signal resampled at 8 kHz
    sig, sampling_rate = librosa.load(file_path, sr=None, mono=True, offset=0)
    # preprocess signal
