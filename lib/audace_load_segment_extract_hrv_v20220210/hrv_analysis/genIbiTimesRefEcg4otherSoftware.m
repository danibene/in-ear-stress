...
resultsFileName = 'TimeLabRefEcg_20220210.xlsx';
%% set constants
...
timeFeatLabels = [{'MeanNN'}];
freqFeatLabels = [{'RatioLFHF'}];
            ...
            for fIdx=1:length(featLabels)
                ...
                for rest=1:length(restVsStressConds)
                    ...
                    for sIdx=possSegIdx
                        ...
                        resultsTable.uniqueSegId(count) = count;
                        if rest == 1
                        ...
                        if ~isnan(resultsTable.Value(count))
                            ...
                            % the start of the ECG recording
                            resultsTable.SegmentStartRefEcg(count) = ...
                                segT1 + finddelayecg2arp(paths,pIdx,...
                                'saveFile','readFromFile');
                            resultsTable.SegmentEndRefEcg(count) = ...
                                segTEnd + finddelayecg2arp(paths,pIdx,...
                                'saveFile','readFromFile');
                            ...
                            segIbiTime = allIbiTime(allIbiTime > ...
                                segT1 & allIbiTime <= segTEnd) + ...
                                finddelayecg2arp(paths,pIdx,...
                                'saveFile','readFromFile');
                            
                            allSegIbi4Table = [allSegIbi4Table; segIbi];
                            allSegIbiTime4Table = [allSegIbiTime4Table; segIbiTime];
                            allUniqueSegId4Table = [allUniqueSegId4Table; count * ones(size(segIbiTime))];
...
resultsTableNew = removevars(resultsTable,{'Value'});

writetable(resultsTableNew,fullfile(paths.hrvFeatDirPath,resultsFileName));
